...
def setup_test(inputs, outputs, activation, out_activ):
    ...
    temp_input = inputs[(4*16):((4*16)+16)]
#        print (temp_input) # only want inputs/outputs in sets of 16 per dimension
    temp_out = outputs[(4*16):((4*16)+16)]
#    out_activ.append(activation[2][0])
    out_activ.append('R')
    logger = logging.getLogger('TEST-%s' % 1)
    logger.setLevel(logging.DEBUG)
    # file write handler
    file_handler = logging.FileHandler('6-1-Results.log')

    # custom formatter
    formatter = logging.Formatter('')
    file_handler.setFormatter(formatter)
    activation = [[['G','G','G']],[['G','G','G','G','G']],[['G','G','G','G','G','G','G']]]
    # register file handler
    logger.addHandler(file_handler)
    start_thread(temp_input, activation[2], out_activ, temp_out, learn_rate, 
