...
class node:
	...
	def addInputs(self, nodes):
		for x in nodes:
			...
			self.weights.append((random.random()*2)-1)		#read somewhere this should be -.5 to .5
	...
	def getWeightForNode(self, node):					#is this different from getWegihts(self)??
	...
	def calcOutputError(self, answer):
		if self.func == 'S':
			self.error = (answer - self.value) * self.value * (1 - self.value)		#error * sigmoid derivative
		elif self.func == 'L': 
			#print(answer, self.value)
			self.error = (answer - self.value)			#error * linear derviative
		elif self.func == 'R':
			self.error = (answer - self.value)			#error * linear derivative
	...
	def updateWeights(self, LearnRate, Momentum, loop):
		...
		if self.func == 'S' or self.func == 'L' or self.func == 'R':
			for i in range(len(self.weights)):
				...
				self.weights[i] = self.weights[i] + (Momentum * (self.weights[i] - self.historicalWeights[i]))		##what is this doing????
...
def main(inputs, arrangement, outputs, answers, learnrate = 0.5, threshold = 1, momentum = 0):
	...
	for i in range(len(answers)):					#answers is a 2D array?
